...
export class LambdaAuthService
  implements AuthService<APIGatewayProxyEvent, APIGatewayProxyResult>
{
  ...
  public constructor(
    ...
    opts?: Partial<LambdaAuthOptions>
  ) {
  ...
  public async getUserId(
    ...
    try {
      const payload: string | JwtPayload = jwt.verify(
        token,
        await this.getTokenSecret()
      );
      return typeof payload === 'object' ? payload['userId'] : undefined;
    } catch (err) {
      throw new Error(`Received invalid token: ${(err as Error).message}`);
    }
  }
  ...
  private async generateIdTokenCookieValue(userId?: string): Promise<string> {
    return cookie.serialize(
      this.opts.tokenCookie,
      await this.createTokenValue(userId),
      {
        httpOnly: true,
        secure: true,
        sameSite: 'lax',
  ...
  private async createTokenValue(userId?: string): Promise<string> {
    ...
    return !userId
      ? ''
      : jwt.sign({ userId }, await this.getTokenSecret(), {
          expiresIn: this.opts.tokenTTLSeconds,
        });
  }
  ...
  private async getTokenSecret(): Promise<string> {
    if (!this.resolvedSecret) {
      this.resolvedSecret =
        typeof this.secret === 'string' ? this.secret : await this.secret();
    }
