...
export interface FileSystemTestConfig {
  ...
  generateFileSystem: (
    rootPath: string,
    entries: NullFileSystemEntry[]
  ) => FileSystem;
}

describe('Placeholder for generic FileSystem tests', () => {
...
export const fileSystemTests = (config: FileSystemTestConfig): void => {
  ...
  const asPath = (partialPath: string): string =>
    resolve(rootPath, partialPath);
  ...
  describe('FileSystem', () => {
    ...
    describe('delete', () => {
      it('deletes an existing file', () => {
        ...
        expect(
          fs.exists(asPath('project/packages/package1/package.json'))
        ).toBeFalsy();
      });
    ...
    describe('read', () => {
      it('reads an existing file', () => {
        expect(fs.read(asPath('project/test.txt'))?.toString()).toBe(
          'Test text.'
        );
      });
    ...
    describe('write', () => {
      it('writes a new file', () => {
        fs.write(
          asPath('project/temp/test.txt'),
          Buffer.from('Empty no more!')
        );
        expect(fs.read(asPath('project/temp/test.txt'))?.toString()).toBe(
          'Empty no more!'
        );
      });
    ...
    describe('touch', () => {
      ...
      it('throws error if parent directories do not exist', () => {
        expect(() =>
          fs.touch(asPath('project/temp/sub/subofsub/test.txt'))
        ).toThrow();
      });
    });
    ...
    describe('copy', () => {
      it('can copy an existing file to a new location', () => {
        ...
        expect(fs.read(asPath('project/temp/copy.txt'))?.toString()).toBe(
          'Test text.'
        );
      });
      it('can copy over an existing file', () => {
        fs.copy(asPath('project/test.txt'), asPath('project/package.json'));
        expect(fs.read(asPath('project/package.json'))?.toString()).toBe(
          'Test text.'
        );
      });
      it('throws error if parent directories do not exist', () => {
        expect(() =>
          fs.copy(asPath('bar/test.txt'), asPath('foo/test.txt'))
        ).toThrow();
      });
      it('recursively copies a directory to a new location', () => {
        fs.copy(asPath('project'), asPath('project2'));
        expect(fs.read(asPath('project2/test.txt'))?.toString()).toBe(
          'Test text.'
        );
